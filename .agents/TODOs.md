...
과거 이력: 초기 Flutter 템플릿 위젯 테스트 통과가 보고되었다. 현재 테스트는
이전 `MyApp`·카운터 화면을 참조하므로 현재 앱의 검증 결과로 사용하지 않는다.

## Phase 0 — 프로젝트 기반

- [x] Flutter 프로젝트와 Android/iOS/Web/Windows 타깃 생성
- [ ] 기존 카운터 테스트를 현재 추적 화면 테스트로 교체하고 분석·테스트 통과
- [x] GPS 위치 스트림 의존성 및 Android/iOS 권한 선언
- [x] 프로젝트 문서·소스 계층 설계 작성
- [ ] 상태 관리 방식 결정·적용
- [ ] 라우팅·로깅·개발/운영 환경 설정 결정
- [ ] 실제 코드를 도메인·데이터·화면 계층으로 분리
...
- [ ] 개인정보·로컬 삭제·Drive 백업 보관 기간 정책 확정
- [ ] 로컬 DB/미디어 암호화와 Export 암호 설정 여부 확정
- [ ] 백업 암호화 결정 기록: 검증된 라이브러리, 키 파생·인증 암호화 방식/매개변수,
...
- [ ] 로컬 고유 ID와 `updatedAt` 정의 및 Import/복원 시 ID 유지
- [ ] 로컬 삭제·이전 백업의 기록 잔존·명시적 백업 삭제 규칙 정의
- [ ] 로컬 저장소 인터페이스와 영속 구현
- [ ] 추적 세션 시작·종료·복구
...
- [x] 추적 시작·중지 UI
- [x] 현재 좌표·정확도 표시
- [ ] 화면의 GPS 플러그인 직접 호출을 위치 서비스 인터페이스로 이동
- [ ] 카카오맵 지도 및 현재 위치 표시
- [ ] 임시 20m 거리 필터를 50m 또는 최대 30초의 도메인 수집 정책으로 교체
- [ ] 정지 상태 수집 주기 완화
