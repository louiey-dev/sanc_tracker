...
- 로컬 삭제는 현재 기기 데이터에서 즉시 제거하며 다음 백업에서 제외한다. 과거 백업은 불변으로 남을 수 있고, 명시적인 원격 백업 삭제에서만 제거한다.
...
- 백업: `cryptography ^2.8.1`과 `cryptography_flutter ^2.3.4`를 사용한다. Argon2id(memory 64 MiB, iterations 3, parallelism 1, output 32 bytes)로 키를 파생하고 AES-256-GCM(12-byte nonce)으로 인증 암호화한다. 백업마다 16-byte 이상 salt와 새 nonce를 생성한다.
